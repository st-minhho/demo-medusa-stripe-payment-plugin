...
abstract class StripeBase extends AbstractPaymentProcessor {
  ...
  async getPaymentStatus(
    ...
    const paymentIntent = await this.stripe_.paymentIntents.retrieve(id, {
      stripeAccount: 'acct_1PxhxuCi34ByKtCq',
    });
  ...
  async initiatePayment(
    ...
    const { email, context: cart_context, currency_code, amount, resource_id, customer } = context;
  ...
  async updatePaymentData(sessionId: string, data: Record<string, unknown>) {
    try {
      ...
      return (await this.stripe_.paymentIntents.update(
        sessionId,
        {
          ...data,
        },
